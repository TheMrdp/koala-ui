 ...
 *
 * `dial` is the E.164 country calling code *without* the leading `+`. A few territories
 * share a calling code (e.g. +1 across NANP countries); each still gets its own row so the
 * flag and name are correct. Sorted alphabetically by `name` at the bottom of the file.
 */
export interface Country {
  /** ISO 3166-1 alpha-2 code, uppercase. Doubles as the flag module key. */
  iso2: string
  /** English country name shown in the picker list. */
  name: string
  /** E.164 calling code without the `+` prefix. */
  dial: string
}
...
/** All countries, sorted alphabetically by English name (locale-aware so accents sort naturally). */
export const COUNTRIES: Country[] = [...RAW].sort((a, b) =>
  a.name.localeCompare(b.name),
)

/** Fast `iso2` → `Country` lookup for resolving a controlled `country` prop. */
